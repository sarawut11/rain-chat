...
export const sendVitaePurchase = async (ctx, next) => {
  try {
    ...
    const tranDetails = new TransactionDetail({
      toAddress: companyRainAddress,
      amount,
      time: moment().utc().unix()
    });
    const transInfo = await transactionService.createTransactionRequest(userInfo.id, Transaction.TYPE.VITAE_RAIN, amount, tranDetails);
