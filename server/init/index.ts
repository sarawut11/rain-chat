...
const COMPANY_STOCKPILE_USERNAME = process.env.COMPANY_STOCKPILE_USERNAME;
...
const initDB = async () => {
  ...
  await query(sql, [COMPANY_USERID, COMPANY_USERNAME, "company@wallet.com", md5(uniqid()), "Company Wallet", User.ROLE.COMPANY, uniqid(), COMPANY_RAIN_ADDRESS]);
  sql = "INSERT INTO user_info (id, username, email, password, name, role, refcode, walletAddress) VALUES (?,?,?,?,?,?,?,?);";
  await query(sql, [COMPANY_STOCKPILE_USERID, COMPANY_STOCKPILE_USERNAME, "company.stockpile@wallet.com", md5(uniqid()), "Company Stockpile", User.ROLE.STOCKPILE, uniqid(), COMPANY_STOCKPILE_ADDRESS]);
